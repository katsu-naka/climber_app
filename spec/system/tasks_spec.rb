...
RSpec.describe 'タスク詳細', type: :system do
  ...
  it 'ログインしたユーザーはタスク詳細ページに遷移してタスクの詳細が確認できる' do
    # ログインする
    visit new_user_session_path   
    ...
    # タスク一覧画面のタスク１に「詳細」ボタンがあることを確認する
    expect(page).to have_link("詳細"), href: task_path(@task1)
    # 詳細ページに遷移する
    visit task_path(@task1)
    # 詳細ページにタスク１の内容が含まれている
    expect(page).to have_content("#{@task1.title}")
    expect(page).to have_content("#{@task1.text}")
    # メッセージ用のフォームが存在する
    expect(page).to have_selector(".message-text-form")
  end
  it 'ログインしていないユーザーは詳細画面にアクセスできない' do
    # トップページに移動する
    visit root_path
    # トップページに詳細ボタンがないことを確認する
    expect(page).to have_no_link("詳細"), href: task_path(@task1)
